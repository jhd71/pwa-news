...
class TodoListWidget {
    ...
    createTodoTile() {
        ...
        if (!espaceSeparator) {
            setTimeout(() => this.createTodoTile(), 1000);
            return;
        }
    ...
    this.updateStats();
    this.checkAndNotifyBackup();
    this.checkTasksNotifications(); // AJOUT
}
...
checkTasksNotifications() {
    ...
    // Vérifier d'abord les tâches en retard
    let overdueCount = 0;
    let todayCount = 0;
        ...
        if (!task.completed && task.dueDate) {
            ...
            // Tâche d'aujourd'hui
                ...
                if (!task.notified) {
                    ...
                    if (task.dueTime) {
                        this.scheduleTimeNotification(task);
                    }
                    task.notified = true;
                }
            }
            // Rappel pour demain
            else if (daysDiff === 1 && !task.notifiedTomorrow) {
                this.showToast(`📅 Demain: ${task.title}`, 4000);
                task.notifiedTomorrow = true;
            }
        }
    });
    ...
    if (overdueCount > 0) {
        setTimeout(() => {
            this.showToast(`🔴 Vous avez ${overdueCount} tâche(s) en retard !`, 8000);
            this.playNotificationSound();
        }, 1000);
    }
    ...
    if (todayCount > 0) {
        setTimeout(() => {
            this.showToast(`📋 ${todayCount} tâche(s) à faire aujourd'hui`, 5000);
        }, 2000);
    }
...
playNotificationSound() {
    // Option 1: Avec un fichier audio local
    const audio = new Audio('sounds/notification.mp3'); // Placez votre fichier dans un dossier sounds
    audio.volume = 0.5;
    audio.play().catch(e => {
        console.log('Impossible de jouer le son:', e);
        // Fallback: son synthétique si le fichier n'existe pas
...
playFallbackSound() {
	...
	renderCalendarView(tasks) {
        ...
        for (let day = 1; day <= lastDayIndex; day++) {
            ...
            // Créer la date en format local pour éviter le décalage
    ...
    saveToLocalStorage() {
        try {
            ...
            // AJOUT : Sync avec Supabase pour les tâches importantes
            this.tasks.forEach(task => {
                if (task.priority === 'high' || (task.dueDate && !task.synced)) {
                    this.syncTaskWithSupabase(task);
                    task.synced = true;
                }
            });
    ...
    showToast(message, duration = 3000) {
		// NOUVELLE MÉTHODE : Synchronisation avec Supabase
    async syncTaskWithSupabase(task) {
        try {
            ...
            const taskData = {
                ...
                background: task.background || 'default',
                local_id: task.id
            };
            ...
            // Si c'est une tâche pour aujourd'hui ou demain, programmer une notification
            if (task.dueDate && !task.completed) {
                await this.scheduleSupabaseNotification(task);
            }
    ...
    async scheduleSupabaseNotification(task) {
        try {
            // Récupérer la subscription push si elle existe
            const subscription = JSON.parse(localStorage.getItem('pushSubscription') || '{}');
            if (!subscription.endpoint) return;
            
            const userId = this.getUserId();
            
            // Envoyer à votre API existante
            await fetch('/api/send-notification.js', {
                method: 'POST',
    ...
    getUserId() {
        ...
        if (!userId) {
            // Utiliser le pseudo du chat s'il existe
            const pseudo = localStorage.getItem('pseudo');
            userId = pseudo || 'user_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
