...
extractPseudoFromEmail(email) {
    ...
    if (input && sendBtn) {
        // Créer une variable pour suivre si l'envoi est en cours
        let isSending = false;
        ...
        const sendMessage = async () => {
            ...
            try {
                ...
                // Conserver le contenu du message
                const messageToSend = content;
                
                // Vider l'input immédiatement
                input.value = '';
                
                // Envoyer le message avant de fermer le clavier
                ...
                if (success) {
                    ...
                    // Fermer le clavier APRÈS un envoi réussi
                    setTimeout(() => {
                        input.blur();
                    }, 100);
                } else {
                    // En cas d'échec, restaurer le message
                ...
                // Réinitialiser le drapeau d'envoi
                isSending = false;
            }
        };
