...
extractPseudoFromEmail(email) {
    ...
    if (input && sendBtn) {
        ...
        const handleSendClick = (e) => {
            ...
            // Obtenir le contenu avant toute manipulation
            ...
            // Forcer immédiatement la fermeture du clavier
            if (document.activeElement === input) {
                input.blur();
            }
            
            // Mettre en cache le message pour l'envoyer
            const messageToSend = content;
            
            // Utiliser setTimeout pour s'assurer que l'envoi se produit après les manipulations DOM
            setTimeout(async () => {
                try {
                    // Vérifier les mots bannis
                    const hasBannedWords = await this.checkForBannedWords(messageToSend);
                    if (hasBannedWords) {
                        input.value = messageToSend; // Remettre le message
            ...
            }, 50); // Délai court pour permettre aux événements DOM de se résoudre
        };

        // Remplacer complètement l'ancien gestionnaire d'événements
