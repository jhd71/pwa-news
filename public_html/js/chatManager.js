...
extractPseudoFromEmail(email) {
    ...
    // Supprimer tout écouteur d'événement existant du bouton d'envoi
    if (sendBtn) {
        const newSendBtn = sendBtn.cloneNode(true);
        sendBtn.parentNode.replaceChild(newSendBtn, sendBtn);
        sendBtn = newSendBtn; // Mettre à jour la référence
    }

    if (input && sendBtn) {
        // Solution directe pour l'envoi de message
        sendBtn.addEventListener('click', async () => {
            // Capturer le contenu avant toute modification
            ...
            try {
                ...
                // Vider l'input AVANT d'envoyer
                input.value = '';
                
                // IMPORTANT: Envoyer le message AVANT de fermer le clavier
                const success = await this.sendMessage(content);
                
                if (success) {
                    // Fermer le clavier APRÈS l'envoi réussi
                    input.blur();
                    this.playSound('message');
                    this.scrollToBottom();
                } else {
                    // En cas d'échec, restaurer le contenu
                    input.value = content;
                    this.playSound('error');
                }
            } catch (error) {
                console.error('Erreur envoi message:', error);
                input.value = content; // Restaurer en cas d'erreur
        ...
        input.addEventListener('keydown', async (e) => {
            if (e.key === 'Enter' && !e.shiftKey) {
                ...
                try {
                    ...
                    if (success) {
                        input.blur(); // Fermer le clavier
                        this.playSound('message');
                        this.scrollToBottom();
                    } else {
                        input.value = content; // Restaurer en cas d'échec
                        this.playSound('error');
                    }
                } catch (error) {
                    console.error('Erreur envoi message (Entrée):', error);
                    input.value = content; // Restaurer en cas d'erreur
                    this.showNotification('Erreur lors de l\'envoi', 'error');
                }
            }
