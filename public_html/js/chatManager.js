...
extractPseudoFromEmail(email) {
    ...
    if (input && sendBtn) {
        ...
        const prepareSendMessage = () => {
            // Ne rien faire si un message est déjà en cours de traitement
            ...
            // Lancer l'envoi après un court délai pour laisser le temps au clavier de se fermer
            setTimeout(processSendMessage, 300);
        };
        
        // Fonction pour traiter l'envoi
        const processSendMessage = async () => {
            if (!pendingMessage || processingMessage) return;
            
            processingMessage = true;
            
            try {
                // Vérifier les mots bannis
                if (await this.checkForBannedWords(pendingMessage)) {
                ...
                // Envoyer le message
                const success = await this.sendMessage(pendingMessage);
                
                if (success) {
                    this.playSound('message');
                    this.scrollToBottom();
                } else {
                    // En cas d'échec, remettre le message dans l'input
                    input.value = pendingMessage;
                    this.playSound('error');
                }
